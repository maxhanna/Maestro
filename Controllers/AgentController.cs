...
Reply ONLY with the JSON array — no explanation, no markdown.";
        ...
        string? cardId = null, bool createTests = false, string? buildCommands = null)
    {
        ...
        if (fixBuildMatch.Success)
        {
            ...
            if (cmds.Count > 0)
            {
                ...
                fixBuildOutput = _terminal.ReadAll();
            }
            var resultSteps = new List<object>();
            await RunRepairPlan(projectRoot, emitSse, ct, prompt, fixBuildOutput ?? "", resultSteps);
        ...
        if (pipelineType == PipelineType.CommandExecution)
        {
            ...
            if (chainedNext == PipelineType.CodeEdit)
            {
                ...
                if (createdFiles.Count > 0)
                {
                    ...
                    var chainResult = await StepResolutionPipeline(prompt, projectRoot, emitSse, ct,
                    ...
                    // "repair" tag StepResolutionPipeline already set internally is preserved.
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //  STEP RESOLUTION PIPELINE  (discover → plan → execute)
    //  (formerly UnifiedPipeline; renamed during the origin/main merge)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    // code around it.
    internal virtual async Task<(List<object> steps, AgentPlan plan, bool complete)> StepResolutionPipeline(
        ...
        string? cardId = null)
    {
        ...
        if (!taskComplete)
        {
            ...
            while (!taskComplete && repairIteration < MaxPostVerifyRepairIterations)
            {
                ...
                if (plan != null)
                {
                    var beforeRepairCount = allSteps.Count;
                    await ExecutePlan(prompt, projectRoot, emitSse, "", plan, ct, allSteps,
                        ...
                        completedStepIndices: mergedDone, cardId: cardId);
                    // Post-execution-verify-driven repair passes are a course change relative
                    // to the originally approved plan — tag so noReplan can detect them.
                    TagStepOrigin(allSteps, beforeRepairCount, "repair");
                }
            ...
            else if (exhaustedWithNoSteps)
            {
                ...
                {
                    plan = freshPlan;
                    var beforeFreshPlanCount = allSteps.Count;
                    ...
                    // A full revert + fresh plan after verification failure is a bigger course
                    // change than a same-plan repair — tag as replan for noReplan gate accuracy.
                    TagStepOrigin(allSteps, beforeFreshPlanCount, "replan");
                }
                else
                {
                    await EmitLog(emitSse, "warn",
