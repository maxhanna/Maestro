...
public partial class AgentController : ControllerBase
{
    ...
    public AgentController(
        ...
        PushNotificationService push, DatabaseService db, IBenchmarkTerminalRunner benchmarkTerminal)
    {
        ...
        _boardData = boardData; _push = push; _db = db; _benchmarkTerminal = benchmarkTerminal;
        ...
        try
        {
            ...
            var (allSteps, plan, complete) = await Orchestrate(
                ...
                steeringContext: isBenchmark
                    ? "This is an edit benchmark. Use only the server-selected fixture and do not claim completion until the artifact is changed and verified."
            ...
            if (isBenchmark)
            {
                var successfulSteps = allSteps.OfType<Dictionary<string, object?>>()
                    ...
                    .ToList();
                var hasFatalStep = allSteps.OfType<Dictionary<string, object?>>()
                ...
                complete = !hasFatalStep && (successfulSteps.Count > 0 || planAlreadyDone);
                editsApplied = AgentProjectUtilities.HasSuccessfulEdits(allSteps);
            }
            await SendSse(Response, "done", new
            {
                summary = plan?.Summary ?? "",
