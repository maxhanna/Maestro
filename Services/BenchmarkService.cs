...
public class BenchmarkService
{
    ...
    private static List<BenchmarkPlanDefinition> BuildPlansCore()
    {
        return new List<BenchmarkPlanDefinition>
        {
            ...
            new()
            {
                Level = 22, Name = "Live Web Test 1: visual-bug check of a served web game",
                Description = "Create a folder called 'benchmark_test_22' at the project root. Inside it, build a small web game and then CHECK IT FOR VISUAL BUGS with the live browser test suite.\n\nSTEP 1 — THE GAME: Serve 'index.html' at / with a heading that says exactly 'Benchmark 22', a short paragraph, a visible score/status display, and an interactive element (a button or canvas) that visibly reacts when clicked. Add a /api/health endpoint that returns JSON {\"status\": \"ok\"}. The server must read its port from the PORT environment variable, defaulting to 8765, so it can be started on a different free port when 8765 is already in use — choose whatever language, runtime, or library you prefer.\n\nSTEP 2 — VISUALLY VERIFY: This is the important part — you must LOOK at the rendered page, not just the source. Use the live browser test to spin up the game and visually confirm the 'Benchmark 22' heading and the interactive element actually render on screen (no visual bugs), and that GET /api/health returns the expected JSON. If the preconfigured port is busy, start the server on a different free port and inspect it there. Report exactly what you saw.",
                AcceptanceChecks =
                [
