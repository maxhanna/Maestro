...
(filesystem + live web test) end-to-end.
...
| `BenchmarkLiveWebTestTests` | 11 | Benchmark 22 (platform-agnostic HTTP server + live browser/API verification + port fallback) and the `LiveUiTest`/`LiveApiTest` check types that bridge benchmarks to the testing suite: live checks actually run `BrowserAutomationService` against the benchmark folder (real HTTP-fallback pass/fail), the `_benchmark_verify` entry point (`EvaluateChecksAsync`) end-to-end on a static fixture, canned-runner pass/fail mapping, and the launcher's port-conflict fallback (busy preferred port → a different free port). |
