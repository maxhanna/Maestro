...
(2,076 runtime test cases per `dotnet test` on Windows) + 29 standalone JS suites
(302 tests), run together via `node tests/js/run-all.js`. The CI workflow builds
...
## 3. Inventory — JS client-logic suites (28 files, 293 tests)
...
| `meeting-bubble.test.js` | 19 | The meeting canvas text helpers: `fitFont` (wall/poster text shrinks to fit its frame, floor guard), `drawSpeechBubble` (single-line quips, five-line cap, **monotonic linear scroll — no bounce**, stays on-screen), and `drawSpiderDetails` (every role's signature accessory draws cleanly). |
...
| JS client logic | 25 files | 267 | Good per-module; `run-all.js` aggregator + CI workflow (§4.2); meeting canvas helpers (bubble layout, poster text fitting, spider accessories) and the chat jump-chip state locked in §3 |
