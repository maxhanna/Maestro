...
**Last audited:** August 12, 2026 · **Suite size:** 94 C# test classes
(1,573 runtime test cases per `dotnet test` on Windows) + 12 standalone JS suites
...
| `NewsServiceTests` | 20 | The **fresh-news digest** behind the "model invents a headline + fake URL" failure: the **news marker** (`LooksLikeNewsQuery`, 14-case theory — news/headline/breaking/trending/"latest AI …" prompts route to the digest, while every generic web prompt already in the suite — "latest release notes of .NET 10", "current weather in London", "recent AI breakthroughs", "latest release notes for weaver" — stays on the plain DuckDuckGo path; rule B requires a news NOUN, so "AI research breakthroughs latest" never routes). The **digest pipeline** (scripted HTTP fixtures for all 9 sources): parses RSS (`<description>` + `content:encoded`), HN Algolia JSON, Reddit search JSON and arXiv Atom; dedup collapses the same story across trackers by normalized URL + title (keeps the richer snippet, never the tracker's URL); round-robin interleave cycles feeds (V1→T2→H1→R1→A1→…); long feed snippets become the summary with zero LLM calls; a **dead feed** degrades to an empty list without killing the digest; **all sources dead** returns a "no fresh items" digest and never throws; a **thin snippet** triggers the article-fetch (AngleSharp `<article>` extraction) + injected-summarizer path. Plus the **`ExecuteWebPlanStep` routing** (2, driven through the real private step with a minimal controller): a news-y prompt returns the digest with a REAL title + URL from the live feed (never an invented `example.com/fakearticle`), a generic prompt stays on DuckDuckGo (`## Results`, no `# AI News`). Regression-checked: neutering the news route fails the routing test while the DDG test still passes. The three web-search call sites (interleaved plan path, command-pipeline `web_search` tool, discovery web step) share one `ExecuteWebSearchAsync` routing helper. |
