...
**Last audited:** August 12, 2026 · **Suite size:** 95 C# test classes
(1,585 runtime test cases per `dotnet test` on Windows) + 13 standalone JS suites
(172 tests), run together via `node tests/js/run-all.js`. The CI workflow builds
...
## 2. Inventory — C# unit/integration suites (95 classes)
...
| `NewsServiceTests` | 23 | The **fresh-news digest** behind the "model invents a headline + fake URL" failure: the **news marker** (`LooksLikeNewsQuery`, 17-case theory — rules A (news/headline/breaking/trending), B (recency + topic + news noun) and **C** (topic + article noun + persist intent — "write/save/paste … AI article … to a file/desktop", which is the EXACT prompt that produced the invented-headline failure and the reason the rule exists)). The **digest pipeline** (scripted HTTP fixtures for all 9 sources): parses RSS (`<description>` + `content:encoded`), HN Algolia JSON, **Lobsters** newest.json (replaced Reddit — Reddit's `search.json` returns 403 to keyless server requests, so it was dead weight; Lobsters is the keyless social source that actually works) and arXiv Atom; dedup collapses same-story across trackers (keeps the richer snippet); round-robin interleave (V1→T2→H1→L1→A1→…); long snippets become the summary with zero LLM calls; a **dead feed** degrades to an empty list; **all sources dead** returns a "no fresh items" digest and never throws; a **thin snippet** triggers article-fetch + injected summarizer. Plus **`ExecuteWebPlanStep` routing** (3): the news-y prompt AND the exact user "…interesting and relevant AI article … write the data into a text file on my desktop" prompt both return the digest with a REAL title + URL (never `example.com/fakearticle`), a generic prompt stays on DuckDuckGo. All live feeds were curl-verified 200 + real content (Reddit 403 was the only dead one) and the digest was smoke-tested live end-to-end. The three web-search call sites share one `ExecuteWebSearchAsync` routing helper. |
