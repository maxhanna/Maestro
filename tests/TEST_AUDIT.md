...
| `TabularFileServiceTests` | 46 | Structural CSV/TSV/XLSX editing (the tabular fast-path): RFC-4180 CSV parse/serialize round-trips (quoting, BOM, CRLF, embedded newlines, `FETCHED_AT:` preamble), TSV, XLSX read/write (shared/inline strings, numeric vs string cells, empty cells, valid ZIP parts), every structured operation (add/remove/rename column, add/delete row, set cell, replace value), and the decline paths (unrecognized op, missing column). |
